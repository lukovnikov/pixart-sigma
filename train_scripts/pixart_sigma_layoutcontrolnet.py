...
class PixArtTransformer2DModelWithLayoutControlNet(PixArtTransformer2DModel):
    ...
    def adapt(cls, main, control_encoder=None, control_encoder2=None, num_layers=-1, use_controlnet=False, use_adapters=False, use_controllora=False, lora_rank=64, use_identlin=False):
        ...
        NUM_OBJ_LAYERS = 25
        T5_DIM = 4096
        main.obj_layer_embed = torch.nn.Embedding(NUM_OBJ_LAYERS, T5_DIM)
    ...
    def forward(
        ...
        obj_layer_embed = self.obj_layer_embed(control_layer_ids)
        obj_layer_embed *= control_encoder_attention_mask.float()[:, :, None]
        control_encoder_hidden_states += obj_layer_embed        
...
class PixArtSigmaLayoutControlNetPipeline(PixArtSigmaPipeline):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        _, _, control_prompt_embeds, control_attention_mask, control_layer_ids = encode_text(example, self.text_encoder)
