...
class VanillaModule(torch.nn.Module): pass


class FeatureGate(VanillaModule):
    init_main_frac = 0.9
...
class AttnMixerLight(VanillaModule):
    def __init__(self, dim, numheads):
        super().__init__()
        self.dim, self.numheads = dim, numheads
...
class CustomAttnProcessor2_0:
    ...
    EXCLUSIVE_ATTENTION_THRESHOLD = 0.99
    ATTEND_TO_ALL_IN_BACKGROUND = False
    ...
    @classmethod
    def adapt(cls, obj, attn, use_attention_embeddings=False, mask_attention=True, attention_mix=False, attention_mix_light=False, attend_to_all_in_background=None):
        obj.__class__ = cls
        NUM_OBJ_LAYERS = 25
        T5_DIM = 1152
        if attention_mix:
        ...
        obj.attend_to_all_in_background = attend_to_all_in_background if attend_to_all_in_background is not None else cls.ATTEND_TO_ALL_IN_BACKGROUND
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if self.mask_attention:
            # make sure attention mask only allows overlap if full-masked region overlap
            is_full = (object_masks > self.EXCLUSIVE_ATTENTION_THRESHOLD)
            any_full = is_full.any(1, keepdim=True)
            _removed_attn = torch.where(any_full, 0, object_masks)
            object_masks = torch.where(is_full, object_masks, _removed_attn)
            # 2. compute attention with object masks
            globalonly = torch.ones_like(object_masks[:, 0:1])
            # globalonly = torch.where(any_full, 0, globalonly)
            object_masks = torch.cat([globalonly, object_masks], 1) 
            object_masks = torch.nn.functional.avg_pool2d(object_masks, 16, 16) > 0
            if self.attend_to_all_in_background:
                is_empty = (object_masks[:, 1:] == 0).all(1, keepdim=True)
                object_masks = torch.where(is_empty, 1, object_masks)       # set attention mask to 1 for pixels that are not described
...
class PixArtTransformer2DModelWithLayoutControlNet(PixArtTransformer2DModel):
    ...
    def adapt(cls, main, control_encoder=None, control_encoder2=None, num_layers=-1, use_controlnet=False, 
              ...
              use_attention_embeddings=False, use_masked_attention=False, 
              use_masked_attention_backbone=False, use_masked_attention_control=False,
              use_attention_mix=False, use_attention_mix_light=False, attend_to_all_in_background=None):
        main.__class__ = cls
            ...
            for block in main.control_blocks:
                if block is not None:
                    CustomAttnProcessor2_0.adapt(block.attn2.processor, block.attn2, use_attention_embeddings=use_attention_embeddings, 
                                                 mask_attention=use_masked_attention and use_masked_attention_control, 
                                                 attention_mix=use_attention_mix, attention_mix_light=use_attention_mix_light, 
                                                 attend_to_all_in_background=attend_to_all_in_background)
                    
            if use_masked_attention and use_masked_attention_backbone:
                for block in main.transformer_blocks:
                    CustomAttnProcessor2_0.adapt(block.attn2.processor, block.attn2, use_attention_embeddings=use_attention_embeddings, 
                                                 mask_attention=use_masked_attention and use_masked_attention_backbone, 
                                                 attention_mix=use_attention_mix, attention_mix_light=use_attention_mix_light,
                                                 attend_to_all_in_background=attend_to_all_in_background)
    ...
    def get_trainable_parameters(self):
        ...
        vanillaparams = []
        def append_to_vanilla_params_if_vanilla(m):
            if isinstance(m, VanillaModule):
                for param in m.parameters():
                    vanillaparams.append(param)
        self.apply(append_to_vanilla_params_if_vanilla)
    ...
    def get_control_state_dict(self):
        ...
        vanilla_module_prefixes = set()
        for k, v in self.named_modules():
            if isinstance(v, VanillaModule):
                vanilla_module_prefixes.add(k)
        
        for k, v in self.state_dict().items():
            if k.split(".")[0] in {"control_encoder", "control_blocks", "zeroconvs", "simple_encoder", "simple_connectors"}:
                ret[k] = v
            if any([k.startswith(_prefix) for _prefix in vanilla_module_prefixes]):
                ret[k] = v
    ...
    def forward(
        ...
        cross_attention_kwargs = {"layer_ids": control_layer_ids, "object_masks": control_image}
