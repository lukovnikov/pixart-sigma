...
from pixart_sigma_layoutcontrolnet import PixArtSigmaLayoutControlNetPipeline, PixArtTransformer2DModelWithLayoutControlNet, create_zeroconv2d, preprocess_example, collate_fn, encode_text
...
def load_data(args, tokenizer, split="train"):
    ...
    dataset.transforms.append(partial(preprocess_example, proportion_empty_prompts=args.proportion_empty_prompts, max_length=max_length, tokenizer=tokenizer))
...
class Validator:
    ...
    def __init__(self, transformer=None, vae=None, text_encoder=None, accelerator=None, examples=None, weight_dtype=torch.float32):
...
def main(args):
    ...
    if accelerator.is_main_process:
        accelerator.init_trackers("pixart-layoutcontrolnet-coco-v2", config=vars(args))
    ...
    while True:
        ...
        with accelerator.accumulate(transformer):
            ...
            main_prompt_embeds, main_attention_mask, control_prompt_embeds, control_attention_mask, control_layer_ids = encode_text(batch, text_encoder)
            ...
            model_pred = transformer(noisy_latents,
                                     control_image=control_image,
                                        encoder_hidden_states=(main_prompt_embeds, control_prompt_embeds, control_layer_ids),
                                        encoder_attention_mask=(main_attention_mask, control_attention_mask),
                                        timestep=timesteps,
...
def mainfire_controlnet(
        train_data_dir="/USERSPACE/lukovdg1/coco2017",
        # use_identlin=True,
        output_dir="/USERSPACE/lukovdg1/pixart-sigma/train_scripts/control_experiments_v2/pixart_coco_layoutcontrolnet",
        # resume_from_checkpoint="latest",
        pretrained_model_name_or_path="PixArt-alpha/PixArt-Sigma-XL-2-512-MS",
        use_controlnet=True,
        control_encoder="/USERSPACE/lukovdg1/pixart-sigma/train_scripts/control_ae_output_v2_controlnet/encoder.pth",
...
#    [ ]: write code to load those captions
