...
from pixart_sigma_layoutcontrolnet_mix import PixArtSigmaLayoutControlNetPipeline, PixArtTransformer2DModelWithLayoutControlNet, create_zeroconv2d, preprocess_example, collate_fn, encode_text
...
from convert_f30ke import generate_dualmask_point, generate_innerouter_mask, generate_dualmask_bbox as generate_bbox, generate_random_precision_map
...
def parse_args(inpargs):
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.,
        help="Probability of using bbox condition for an object instead of mask.",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.,
        help="Probability of using variable precision condition for an object instead of original mask.",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.,
        help="Probability of using point condition for an object instead of original mask.",
    )
    ...
    parser.add_argument(
        "--mask_attention_backbone",
        default=False,
        action="store_true",
        help=(
            "Whether to apply simple attention masking in cross-attention layers in the backbone model."
        ),
    )
    parser.add_argument(
        "--mask_attention_control",
        default=False,
        action="store_true",
        help=(
            "Whether to apply simple attention masking in cross-attention layers in the control model."
        ),
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Whether to use learnable attention mix instead of just masking attention."
        ),
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Whether to enable pixels that don't have any tokens associated with them to attend to all tokens."
        ),
    )
...
def multi_mix(example, 
              bbox_prob=0., varprec_prob=0., point_prob=0., 
              bbox_level=0.5, varprec_levels=(0.5, 1.), pointlevels=(0.25, 1.), device=torch.device("cpu")):
    assert(bbox_prob + varprec_prob + point_prob <= 1.)
    normal_prob = 1 - bbox_prob - varprec_prob - point_prob
    whichtransform = random.choices(["none", "bbox", "varprec", "point"], weights=[normal_prob, bbox_prob, varprec_prob, point_prob], k=len(example["seg_captions"]))
    rmap = None
    for i in range(len(example["seg_captions"])):
        mask = example["cond_image"][i]
        if mask.sum().item() > 0:
            if whichtransform[i] == "varprec":
                if rmap is None:
                    rmap = generate_random_precision_map(mask.shape, clampmin=0.2)
                newmask = generate_innerouter_mask(mask, rmap=rmap, outerboxlevel=varprec_levels[0], innerboxlevel=varprec_levels[1], device=device)
                example["cond_image"][i] = newmask
            elif whichtransform[i] == "bbox":
                example["cond_image"][i] = generate_bbox(mask, outerboxlevel=bbox_level)
            elif whichtransform[i] == "point":
                example["cond_image"][i] = generate_dualmask_point(mask, outerboxlevel=pointlevels[0], innerboxlevel=pointlevels[1])
                
    return example
    

NUMOBJ = 20
def load_data(args, tokenizer, split="train", device=torch.device("cpu")):
    # See Section 3.1. of the paper.
    max_length = args.max_token_length
    
    # if accelerator.is_main_process:
    dataset = COCOInstancesDataset(maindir=args.train_data_dir, split=split, upscale_to=512, max_masks=NUMOBJ)
    dataset.transforms.append(partial(multi_mix, bbox_prob=args.bbox_mix, varprec_prob=args.varprec_mix, point_prob=args.point_mix))
...
def load_pretrained(path, device=torch.device("cuda"), return_pipe=True, load_saved_state_dict=True, _override_args=None):
    path = Path(path)
    with open(path / "args.json") as f:
        args = argparse.Namespace(**json.load(f))
        
    if _override_args is not None:
        for k, v in _override_args.items():
            setattr(args, k, v)
...
def setup_accelerator(args, logging_dir):
    ...
    accelerator = Accelerator(
        ...
        cpu=default(lambda: args.cpu_only, False),
    )
...
def adapt_transformer_controlnet(transformer, args=None, control_encoder=None, control_encoder2=None, num_layers=-1, _return_trainable=False, weight_dtype=torch.float32, use_attention_embeddings=False):
    ...
    transformer = PixArtTransformer2DModelWithLayoutControlNet.adapt(transformer, control_encoder=control_encoder, control_encoder2=control_encoder2, 
                                                                     ...
                                                                     use_masked_attention_backbone=default(lambda: args.mask_attention_backbone, False),
                                                                     use_masked_attention_control=default(lambda: args.mask_attention_control, False),
                                                                     ...
                                                                     attend_to_all_in_background=default(lambda: args.no_token_use_all, None),
                                                                     )
...
def main(args):
    ...
    train_dataset, train_dataloader = load_data(args, tokenizer, split="train", device=accelerator.device)
    ...
    if accelerator.is_main_process:
        accelerator.init_trackers("pixart-layoutcontrolnet-mix-coco", config=vars(args))
...
def get_actual_args(fargs, kwargs):
    ...
    actualargs = {}
    actualargs.update(fargs)
    actualargs.update(kwargs)
        
    actualargstrings = []
    
    for k, v in actualargs.items():
        if v is True:
            actualargstrings.append(f"--{k}")
        else:
            actualargstrings.append(f"--{k}={v}")
            
    args = parse_args(actualargstrings)
    return args


  
def mainfire_controlnet_varprec_noattnctrl(
        train_data_dir="/USERSPACE/lukovdg1/coco2017",
        # bbox_mix=0.5,
        # bbox_mix=0.2,
        varprec_mix=0.8,
        no_masked_attention=True,
        output_dir="/USERSPACE/lukovdg1/pixart-sigma/train_scripts/control_experiments_v2/pixart_coco_layoutcontrolnet3_varprec_noattnctrl",
        ...
        **kwargs,
        ...
        main(get_actual_args(locals().copy(), kwargs))
  
def mainfire_controlnet_varprec_attnmaskboth(
        train_data_dir="/USERSPACE/lukovdg1/coco2017",
        # bbox_mix=0.5,
        # bbox_mix=0.2,
        varprec_mix=0.8,
        ...
        output_dir="/USERSPACE/lukovdg1/pixart-sigma/train_scripts/control_experiments_v2/pixart_coco_layoutcontrolnet3_varprec_attnmaskboth",
        pretrained_model_name_or_path="PixArt-alpha/PixArt-Sigma-XL-2-512-MS",
        use_controlnet=True,
        omit_global_caption=True,
        control_encoder="/USERSPACE/lukovdg1/pixart-sigma/train_scripts/control_ae_output_v2_controlnet/encoder.pth",
...
def mainfire_controlnet_bboxmask_noattnctrl(
        train_data_dir="/USERSPACE/lukovdg1/coco2017",
        bbox_mix=0.5,
        no_masked_attention=True,
        output_dir="/USERSPACE/lukovdg1/pixart-sigma/train_scripts/control_experiments_v2/pixart_coco_layoutcontrolnet_mix_bboxmask_noattnctrl",
        pretrained_model_name_or_path="PixArt-alpha/PixArt-Sigma-XL-2-512-MS",
        use_controlnet=True,
        omit_global_caption=True,
        control_encoder="/USERSPACE/lukovdg1/pixart-sigma/train_scripts/control_ae_output_v2_controlnet/encoder.pth",
...
def mainfire_controlnet_bboxmask_attnmaskboth(      # train with attention control enabled both in backbone and control
        train_data_dir="/USERSPACE/lukovdg1/coco2017",
        bbox_mix=0.5,
        mask_attention_backbone=True,
        mask_attention_control=True,
        no_token_use_all=True,
        output_dir="/USERSPACE/lukovdg1/pixart-sigma/train_scripts/control_experiments_v2/pixart_coco_layoutcontrolnet_mix_bboxmask_attnmaskboth",
        pretrained_model_name_or_path="PixArt-alpha/PixArt-Sigma-XL-2-512-MS",
        use_controlnet=True,
        omit_global_caption=True,
        control_encoder="/USERSPACE/lukovdg1/pixart-sigma/train_scripts/control_ae_output_v2_controlnet/encoder.pth",
...
def mainfire_controlnet_bboxmask_attnmixboth(      # train with attention control enabled both in backbone and control
        train_data_dir="/USERSPACE/lukovdg1/coco2017",
        bbox_mix=0.5,
        mask_attention_backbone=True,
        mask_attention_control=True,
        use_attention_mix=True,
        no_token_use_all=True,
        output_dir="/USERSPACE/lukovdg1/pixart-sigma/train_scripts/control_experiments_v2/pixart_coco_layoutcontrolnet_mix_bboxmask_attnmixboth",
        pretrained_model_name_or_path="PixArt-alpha/PixArt-Sigma-XL-2-512-MS",
        use_controlnet=True,
        omit_global_caption=True,
        control_encoder="/USERSPACE/lukovdg1/pixart-sigma/train_scripts/control_ae_output_v2_controlnet/encoder.pth",
...
# DONE: try layoutcontrolnet with attention mix --> DONE: maybe we also need attention mix in backbone?
# DONE?: finish training bbox layoutcontrolnet with attention mix
...
# NOTYET?: try light attention-mix

# MIX: general
# DONE: make sure that CA mask can overlap only when only full-mask regions can overlap. If a full-mask region of one object overlaps half-mask region of another, the half-mask region's CA mask should be zero there.
# ????: should we also do this for conditioning object masks themselves?
# TODO: fix image quality. Image quality issues seem to arise from the way localized text conditioning is implemented. Need to think of better ways.

# MIX: BBOX/MASK
# DONE: implement bbox/mask + attention-mask
# DONE: train bbox/mask + attention-mask  --> works (small masks are not followed precisely though), big problem is quality degradation
# PROBLEM: background is white, maybe because there is no global prompt? --> no, it's because the text conditioning is breaking things
# DONE: global prompt masking
# PROBLEM: image quality degrades, too much contrast
# DONE: train bbox/mask + attnmask without text : image quality degradation less severe, no contrast issues, but grass has a tiley texture
# DONE: check if simple attention erase in both backbone and control works --> it works, but backgrounds are bad because they have no tokens to attend to
# OBSERVATION: enabling attention to all tokens in pixels not covered by any mask
# DONE: train with full simple attention erase (with or without all tokens for background? --> with, because no-token pixels know they are no-token and should learn to use object tokens only for background info purposes)
# ↳ DONE: compare with regular training: same as attnboth during test only, but alllowing attending to all tokens when pixel is not in a mask enables better backgrounds. However, trained variant is better since then there are no instances where a described object is generated as part of the background.
# DONE: train with full attention mix, trainable  (was running out of memory, so using gradient checkpointing)
# ↳ not necessarily better, misses gold coin (yellow tennis ball instead) --> prob not worth it

# MIX: varprec
# DONE: implement varprec mix (done) and test 
# MAYBE: make sure that CA mask doesn't allow overlapping conditions in inner-region --> but maybe not because overlapping of conditions should also be ok.
# DONE: make varprec efficient in sampling during training
# DONE: train varprec without attnmask
# TODO: train varprec+bbox+point
# TODO: train varprec with attnmask everywhere during training
# TODO: create clear examples for varprec to see how effective it is
# ↳ TODO: horse examples

# MIX: text conditioning
# DONE: fix the no-token background problem --> no_token attend-all seems to work ok
# ↳ DONE: try to generate with all tokens available when no other mask covers it  --> makes the backgrounds less dull but object can bleed over their bounds
# TODO[THINK]: how can we bind text to the instance mask better so there's less risk for concept bleeding with overlapping bboxes etc?

# TODO[READ]: read MIGC++, DiffX, MS-Diffusion (citations of InstanceDiffusion)
# TODO: does training with attnmaskboth actually improve quality?

# Difficult cases:
# - two elephants with overlapping/touching masks --> early stages of bbox/mask were giving a big merged elephant



if __name__ == "__main__":
    # fire.Fire(mainfire_controlnet_bboxmask_noattnctrl)
    # fire.Fire(mainfire_controlnet_bboxmask_attnmaskboth)
    # fire.Fire(mainfire_controlnet_bboxmask_attnmixboth)
    # fire.Fire(mainfire_controlnet_varprec_noattnctrl)
    fire.Fire(mainfire_controlnet_varprec_attnmaskboth)